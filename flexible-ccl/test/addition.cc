...
int main(int argc, char *argv[]) {
    ...
    // Measure first communication time
    clock_gettime(CLOCK_MONOTONIC, &start);
    NCCLCHECK(ncclGroupStart());
    for (int i = 0; i < nFirst; ++i)
...

    {
        ...
        nanoseconds = end.tv_nsec - start.tv_nsec;
        long elapsed = (seconds * 1000000) + (nanoseconds / 1000);
        printf("FIRST COMMUNICATION TIME: %ld us (%.3f ms)\n", elapsed, elapsed / 1000.0);
    }
    ...
    // Measure subsequent communication time after adding new rank
    clock_gettime(CLOCK_MONOTONIC, &start);
    NCCLCHECK(ncclGroupStart());
    for (int i = 0; i < nTotal; ++i)
...

    {
        ...
        nanoseconds = end.tv_nsec - start.tv_nsec;
        long elapsed = (seconds * 1000000) + (nanoseconds / 1000);
        printf("SUBSEQUENT COMMUNICATION TIME: %ld us (%.3f ms)\n", elapsed, elapsed / 1000.0);
    }
