...
static ncclResult_t ncclCommInitRankFunc(struct ncclAsyncJob* job_) {
  ...
  // For traditional initialization, activate all connectors
  // This ensures backward compatibility with existing NCCL behavior
  for (int c = 0; c < comm->nChannels; c++) {
    struct ncclChannel* channel = comm->channels + c;
    for (int p = 0; p < comm->nRanks; p++) {
      if (channel->peers[p] == NULL) continue;
      // Activate send connectors
      for (int t = 0; t < NCCL_MAX_CONNS; t++) {
        if (channel->peers[p]->send[t].connected) {
          channel->peers[p]->send[t].isActive = 1;
        }
      }
      // Activate receive connectors
      for (int t = 0; t < NCCL_MAX_CONNS; t++) {
        if (channel->peers[p]->recv[t].connected) {
          channel->peers[p]->recv[t].isActive = 1;
        }
      }
    }
  }
...
NCCL_API(ncclResult_t, ncclActivateConnectors, ncclComm_t comm, const int* ranks, int nranks);
ncclResult_t ncclActivateConnectors(ncclComm_t comm, const int* ranks, int nranks) {
  NVTX3_FUNC_RANGE_IN(nccl_domain);

  NCCLCHECK(CommCheck(comm, "ActivateConnectors", "comm"));
  NCCLCHECK(PtrCheck((void*)ranks, "ActivateConnectors", "ranks"));

  if (nranks <= 0) {
    WARN("ActivateConnectors: nranks must be positive, got %d", nranks);
    return ncclInvalidArgument;
  }

  NCCLCHECK(ncclCommEnsureReady(comm));

  // Create a set of ranks to activate for efficient lookup
  bool* rankSet = (bool*)calloc(comm->nRanks, sizeof(bool));
  if (rankSet == NULL) {
    WARN("ActivateConnectors: Failed to allocate memory for rank set");
    return ncclSystemError;
  }

  // Mark the ranks to activate
  for (int i = 0; i < nranks; i++) {
    if (ranks[i] < 0 || ranks[i] >= comm->nRanks) {
      WARN("ActivateConnectors: Invalid rank %d, must be in range [0, %d)", ranks[i], comm->nRanks);
      free(rankSet);
      return ncclInvalidArgument;
    }
  ...
  // Activate connectors for the specified ranks
  for (int c = 0; c < comm->nChannels; c++) {
    struct ncclChannel* channel = &comm->channels[c];
    
    // Activate send connectors
    for (int p = 0; p < comm->nRanks; p++) {
      if (channel->peers[p] != NULL) {
        struct ncclConnector* conn = &channel->peers[p]->send[0];
        if (conn->connected && rankSet[p]) {
          conn->isActive = 1;
          INFO(NCCL_INIT, "Activated send connector for channel %d, peer %d", c, p);
        }
      }
    }
    ...
    for (int p = 0; p < comm->nRanks; p++) {
      if (channel->peers[p] != NULL) {
        struct ncclConnector* conn = &channel->peers[p]->recv[0];
        if (conn->connected && rankSet[p]) {
          conn->isActive = 1;
          INFO(NCCL_INIT, "Activated receive connector for channel %d, peer %d", c, p);
        }
      }
    }
  }
  ...
  free(rankSet);
  INFO(NCCL_INIT, "Successfully activated connectors for %d ranks", nranks);
  return ncclSuccess;
}
