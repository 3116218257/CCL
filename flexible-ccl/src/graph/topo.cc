...
ncclResult_t ncclTopoGetSystemForNew(struct ncclComm* comm, struct ncclTopoSystem** system) {
  ...
  // Add GPU nodes for all existing ranks in the communicator
  for (int r = 0; r < comm->nRanks; r++) {
    char busId[NVML_DEVICE_PCI_BUS_ID_BUFFER_SIZE];
    NCCLCHECKGOTO(int64ToBusId(comm->peerInfo[r].busId, busId), ret, fail);
    struct ncclXmlNode* node;
    NCCLCHECKGOTO(ncclTopoFillGpu(xml, busId, &node), ret, fail);
    if (node) {
      NCCLCHECKGOTO(xmlSetAttrInt(node, "rank", r), ret, fail);
      NCCLCHECKGOTO(xmlInitAttrInt(node, "gdr", comm->peerInfo[r].gdrSupport), ret, fail);
    }
  }
